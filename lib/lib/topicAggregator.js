...
// Lazy Supabase client creation (works in both browser and Node.js)
...
const getSupabase = () => {
    ...
    if (typeof import.meta !== 'undefined' && import.meta.env) {
        ...
        const key = import.meta.env.VITE_SUPABASE_ANON_KEY || import.meta.env.SUPABASE_SERVICE_ROLE_KEY;
        if (url && key) {
 ...
 */
export async function aggregateTopicsForUser(userId, subject, examContext) {
    ...
    try {
        ...
        if (examTopics.length === 0)
            return [];
        // 3. Get user's scans (may be empty for new users)
        ...
        let questions = [];
        let questionTopicMap = new Map(); // questionId -> topicId
        if (hasScans) {
            ...
            // Get topic mappings for these questions
        ...
        const existingMap = new Map((existingResources || []).map(r => [r.topic_id, r]));
 ...
 */
export async function getTopicResourceLibrary(userId, topicId) {
    const supabase = getSupabase();
    try {
 ...
 */
export async function calculateTopicMastery(topicResourceId) {
    const supabase = getSupabase();
    try {
        ...
        const mastery = Math.min(100, accuracy * 0.65 +
 ...
 */
export async function recordTopicActivity(userId, topicResourceId, activityType, questionId, isCorrect, timeSpent) {
    const supabase = getSupabase();
    try {
        // Insert activity
 ...
 */
function transformInsight(dbInsight) {
    return {
