/**
 * Build NEET Physics Identity Bank from 2021-2025 actual data
...
async function buildPhysicsIdentities() {
  console.log('\n⚛️ BUILDING NEET CHEMISTRY IDENTITY BANK (2021-2025)\n');
  ...
  for (const [year, scanId] of Object.entries(NEET_SCANS_2021_2025)) {
    ...
    console.log(`📅 ${year}: ${questions?.length || 0} Physics questions`);
  ...
  const identities = sortedTopics.slice(0, 30).map((t, idx) => {
    const id = `ID-NP-${String(idx + 1).padStart(3, '0')}`;
  ...
  fs.writeFileSync(
    path.join(process.cwd(), 'lib/oracle/identities/neet_physics.json'),
  ...
  const calibrationData = {
    version: '2.0',
    description: 'Calibrated identity confidences for NEET Physics based on 2021-2025 empirical data',
    subject: 'Physics',
    exam: 'NEET',
    years_analyzed: 5,
    total_questions: 245,
  ...
  fs.writeFileSync(
    path.join(process.cwd(), 'docs/oracle/calibration/identity_confidences_neet_physics.json'),
    JSON.stringify(calibrationData, null, 2)
  );

  console.log('\n✅ Identity bank saved to: lib/oracle/identities/neet_physics.json');
  console.log('✅ Calibration saved to: docs/oracle/calibration/identity_confidences_neet_physics.json');
  console.log(`\n📈 STATISTICS:`);
