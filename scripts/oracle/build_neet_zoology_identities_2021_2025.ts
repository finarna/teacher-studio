/**
 * Build NEET Zoology Identity Bank from 2021-2025 actual data
...
async function buildZoologyIdentities() {
  ...
  for (const [year, scanId] of Object.entries(NEET_SCANS_2021_2025)) {
    ...
    console.log(`📅 ${year}: ${questions?.length || 0} Zoology questions`);
  ...
  const identities = sortedTopics.slice(0, 30).map((t, idx) => {
    const id = `ID-NZ-${String(idx + 1).padStart(3, '0')}`;
  ...
  fs.writeFileSync(
    path.join(process.cwd(), 'lib/oracle/identities/neet_zoology.json'),
  ...
  const calibrationData = {
    version: '2.0',
    description: 'Calibrated identity confidences for NEET Zoology based on 2021-2025 empirical data',
    subject: 'Zoology',
    exam: 'NEET',
    years_analyzed: 5,
    total_questions: 245,
  ...
  fs.writeFileSync(
    path.join(process.cwd(), 'docs/oracle/calibration/identity_confidences_neet_zoology.json'),
    JSON.stringify(calibrationData, null, 2)
  );

  console.log('\n✅ Identity bank saved to: lib/oracle/identities/neet_zoology.json');
  console.log('✅ Calibration saved to: docs/oracle/calibration/identity_confidences_neet_zoology.json');
  console.log(`\n📈 STATISTICS:`);
