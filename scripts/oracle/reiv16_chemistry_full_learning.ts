...
async function runFullLearning() {
    ...
    console.log('\n🧠 Calculating Recursive Weight Correction (RWC) for Chemistry...');
    
    // ... Training logic simplified for this script ...
    const finalConfig = {
        exam_context: EXAM_CONTEXT,
        subject: SUBJECT,
        rigor_drift_multiplier: 1.15, 
        synthesis_weight: 0.60,
        trap_density_weight: 0.75,
        speed_requirement_weight: 0.85,
        ids_baseline: 0.88,
    ...
    const forecast = {
        ...
        target_year: 2026,
        rigor_velocity: 1.15,
        intent_signature: { synthesis: 0.65, trapDensity: 0.80, linguisticLoad: 0.45, speedRequirement: 0.85 },
        board_signature: 'SYNTHESIZER',
        calibration_directives: [
            "Inject Inorganic Trend Prediction traps",
            "Focus on Organic Yield & VSEPR geometry",
            "Target IDS 0.88 Forensic match"
        ],
        updated_at: new Date().toISOString()
    };
    ...
    console.log('✅ Chemistry Brain Sealed & 2026 Forecast Initialized.');
}
