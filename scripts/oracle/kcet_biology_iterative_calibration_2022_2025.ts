/**
 * KCET Biology Iterative Calibration (2022-2025)
 * REI v17 - Full 60-Question Paper Generation & Calibration
 *
 * This script:
 * 1. Uses 2022 as baseline
 * 2. For each year (2023-2025):
 ...
 *
 * Note: Using 4 years (2022-2025) as 2021 Biology paper not available in database
 */
...
// Official Biology scans (using system scans for consistency)
...
async function runIterativeCalibration() {
  ...
  // Phase 1: Extract 2022 baseline
  console.log('📊 Phase 1: Extracting 2022 Baseline\n');
  const baseline2022 = await extract2022Baseline(identityBank.identities);

  console.log(`   ✓ 2022 IDS Actual: ${baseline2022.idsActual.toFixed(3)}`);
  console.log(`   ✓ Identity Vector: ${Object.keys(baseline2022.identityVector).length} unique identities`);
  console.log(`   ✓ Total Questions: ${baseline2022.totalQuestions}\n`);
  ...
  // Phase 2-4: Calibrate years 2023-2025 (2022 is baseline)
  const years = [2023, 2024, 2025];

  for (const year of years) {
    console.log(`\n${'='.repeat(60)}`);
    console.log(`🔄 Phase ${year - 2021}: Calibrating Year ${year}`);
  ...
  const reportPath = path.join(outputDir, 'KCET_BIOLOGY_CALIBRATION_REPORT_2022_2025.md');
  ...
  const updatedEngineConfig = {
    ...
    calibration_note: 'Calibrated using iterative RWC (2022-2025, 4 years)'
  };
  ...
  console.log(`   - Iteration Logs: ${outputDir}/KCET_BIOLOGY_*_ITERATION_LOG.md\n`);
}

/**
 * Extract 2022 baseline data
 */
async function extract2022Baseline(identities: any[]) {
  console.log('   Fetching 2022 paper from database...');

  const { data: questions } = await supabase
    .from('questions')
    .select('text, topic, difficulty, options, correct_option_index, solution_steps')
    .eq('scan_id', OFFICIAL_SCANS[2022])
    .order('question_order');

  if (!questions || questions.length === 0) {
    throw new Error('Failed to fetch 2022 paper from database');
  }

  console.log(`   ✓ Fetched ${questions.length} questions from 2022 paper`);
  console.log('   Running AI audit on 2022 paper...');
  ...
  if (!audit) {
    throw new Error('Failed to audit 2022 paper');
  }
