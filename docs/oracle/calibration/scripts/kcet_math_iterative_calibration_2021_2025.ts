 ...
 * REI v16 - Full 60-Question Paper Generation & Calibration
 *
 * This script:
...
async function analyzeQuestionTypesFromActualPapers(): Promise<{
  ...
  for (const [year, scanId] of Object.entries(OFFICIAL_SCANS)) {
    ...
    const distPct: Record<string, number> = {
      word_problem: Math.round((dist.word_problem / total) * 100),
      pattern_recognition: Math.round((dist.pattern_recognition / total) * 100),
      computational: Math.round((dist.computational / total) * 100),
      property_based: Math.round((dist.property_based / total) * 100),
      abstract: Math.round((dist.abstract / total) * 100)
    };
